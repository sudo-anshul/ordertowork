...
- An instance profile with SSM connectivity; ECR authorization plus pull actions on the single release repository; S3 attachment/session-prefix permissions; `s3:PutObject` on `backups/*`; and invocation permission for the selected Bedrock endpoint. Mantle uses project/model-scoped permissions described in [the Mantle runbook](mantle-README.md). Runtime uses the selected inference profile and its destination model ARNs. Do not attach administrator access to the runtime instance.
...
Bedrock account authorization is independent of IAM permission. The selected model is `qwen.qwen3-235b-a22b-2507` through Mantle; the example configuration selects it explicitly. Existing configurations without `OTW_BEDROCK_ENDPOINT` retain `runtime` behavior. Changing endpoints does not change `OTW_AGENT_MODE=bedrock`, global/guest paid-attempt limits, or the application stop switch. When AWS denies model access, the deployment records a failed analysis. It must not be presented as successful live AI or silently replaced with reference output.
